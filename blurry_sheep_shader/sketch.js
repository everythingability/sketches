...
function draw() {
	...
	ellipseMode(CENTER);
      imageMode(CENTER)
	fill(random(255),100,random(255),200);
	let size = min(width,height) * random(0.2,0.5);
	//ellipse(random(-width/2,width/2),random(-height/2,height/2),size,size,50);
    image(img, random(-width/2,width/2),random(-height/2, size, size * 1.2) )
	this.shader(sh);
	}
...
function gotFile(file) {
 //Doesn't bloody work!!!
  ...
  temp.onload = function() {
     ...
     img.drawingContext.drawImage(temp, 0, 0);
 ...
 // else  print(`"${file.name}" isn't an image file!`);
  //print( "crossOrigin:", img3.crossOrigin);
