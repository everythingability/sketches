// Boid ( a.k.a. rainbow flowers) class with flocking behavior
class Boid {
  constructor(pos, vel) {
    ...
    this.h = random(); // hue color of the boid
    this.rotZ = random(0.005, 0.02);
    this.ampZ = random(2, 4);
    this.rotY = random(0.01, 0.02);
    this.rotX = random(0.01, 0.02);
  }
  ...
  show() {
    ...
    noStroke();
    fill(1);
    ambientLight(0, 1, 0.03);
    ambientMaterial(this.h, 0.75, 1);
    ...
    let fc = frameCount;
    rotateX(0.2 * cos(fc * this.rotX));
    rotateY(PI + 0.2 * cos(fc * this.rotY));
    rotateZ(this.ampZ * sin(PI/2 * cos(fc * this.rotZ)));
    //scale(1.5);
    model(petals);
    ambientMaterial(0.39, 1, 0.6);
    scale(0.5);
    model(stick);
