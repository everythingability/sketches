...
function fitTextToRect(theText, theWidth, theHeight) {
  ...
  // SETUP A BUFFER FOR THIS PROCESS
  var buffer = createGraphics(theWidth, theHeight)
  buffer.textFont(font)
  buffer.textSize(60)//starting point
  buffer.textStyle(BOLD)
  buffer.textAlign( CENTER, CENTER )
  buffer.imageMode(CENTER);
  //END SET UP
  ...
  lines = []
  runningY = 0
  var leading = buffer.textLeading()
  ...
  while (runningY < theHeight - (leading)) {
    //background("white")
    var asc = buffer.textAscent()
    leading = buffer.textLeading()
    ...
    buffer.textSize(textSize() + 1)

  }
  var tSize = buffer.textSize()
